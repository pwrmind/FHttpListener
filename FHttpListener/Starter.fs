...
// Простой JSON-сериализатор: достаточно для Map<string,_> и списков.
let rec jsonOf (o: obj) : string =
    match o with
    | :? string as s -> "\"" + s.Replace("\\","\\\\").Replace("\"","\\\"") + "\""
    | :? int    as n -> string n
    | :? bool   as b -> if b then "true" else "false"
    | :? DateTime as d -> "\"" + d.ToString("o") + "\""
    | :? System.Collections.IEnumerable as xs ->
        "[" + (xs |> Seq.cast<obj> |> Seq.map jsonOf |> String.concat ",") + "]"
    | _ -> "null"

let private viewToJson (o: obj) : string =
    match o with
    | :? Views.ItemsView as v ->
        let items = v.Items |> List.map (fun i -> box (Map [ "id", box i.Id; "title", box i.Title ]))
        "{\"items\":" + jsonOf (box items) + "}"
    | _ -> "{}"
...
let jsonResponder : Responder =
    fun result _req ->
        match result with
        | Ok (Show data)  -> jsonBody 200 (viewToJson data)
        | Ok (Redirect url) ->
            { Status = 302; Headers = Map [ "Location", url ]; Body = "" }
        | Error e ->
            jsonBody (httpStatus e) ("{\"error\":\"" + errorMessage e + "\"}")
